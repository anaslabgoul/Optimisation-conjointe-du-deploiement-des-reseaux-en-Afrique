...
model.O = pyo.Set(initialize=O)  # offres
model.S = pyo.Set(initialize=Si) # sites de l’opérateur i

# Couples utiles
C_space = list(product([0,1], repeat=1+len(I)))  # Toutes les combinaisons de couverture
...
model.Zmax = pyo.Param(model.T)
model.QA = pyo.Param(model.T)
model.ua0 = pyo.Param(model.A, model.I, model.O)
model.DNG = pyo.Param()
model.CAPANG = pyo.Param()
...
model.u_site = pyo.Var(model.T, model.A, model.S, within=pyo.NonNegativeIntegers)
...
def delta_implication(m, t, a, C):
    ...
    # opérateur τ (index 0)
    cτ = C[0]
    terms.append((m.r[t, a] if cτ==1 else (1 - m.r[t, a])))
    # autres opérateurs
    for k, i in enumerate(m.I):
        ck = C[k]
        terms.append((m.Rcomp[t, a, i] if ck==1 else (1 - m.Rcomp[t, a, i])))

    # Linearisé via big-M :
    return m.delta[t, a, C] <= min(terms)
...
def cov_pop(m, t):
    return sum(ua[a] * m.r[t, a] for a in model.A) >= model.QA[t] * sum(ua[a] for a in model.A)
