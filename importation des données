...
A = list(pd.read_csv("areas_instance.csv")["AREAS"])  # zones
...
Si = list(pd.read_csv("Existing_sites_instance.csv")["EXISTING_SITES"]) # sites de l’opérateur τ

# Couples utiles
C_space = list(product([0,1], repeat=len(I)))  # Toutes les combinaisons de couverture
df = pd.read_csv("Areas_sites_link_instanc.csv")   # contient colonnes "a" et "s"
...
df = pd.read_csv("Areas_sites_link_instance.csv")   # contient colonnes "a" et "s"
...
# 2. PARAMÈTRES
df_zmax = pd.read_csv("OPERATIONAL_LIMITS.csv")
Zmax_data = {row.TIME_SLOTS: row["MAX_NUMBER_OF_DEPLOYMENTS"] for _,row in df_zmax.iterrows()}

df_qa = pd.read_csv("STRATEGIC_GUIDELINES.csv")
QA_data = {row.TIME_SLOTS: row["QA"] for _,row in df_qa.iterrows()}

df = pd.read_csv("areas_instance.csv")

ua0_data = {}
for _, row in df.iterrows():
    for col in df.columns [1:]:
        ua0_data[row["AREAS"],col.split("-")[1], col] = math.floor(row[col])
        
df_dng = pd.read_csv("DEMAND.csv")

DNG_data = {row.TIME_SLOTS: row["5G"] for _,row in df_dng.iterrows()}

df_capang = pd.read_csv("CAPACITY.csv")
CAPANG_data = {row.TIME_SLOTS: row["5G"] for _,row in df_capang.iterrows()}

df_ua = pd.read_csv("areas_instance.csv")
u_a_data = {}
for _, row in df.iterrows():
    somme =0
    for col in df.columns [1:]:
