...
for _, row in df_fdata.iterrows():
    ...
    f_data[tuple(C), O, O2] = row["PERCENTAGES"]
...
for C in C_space:  # tuples de 0/1
    for O1 in O_full:    # toutes les offres existantes
        for O2 in O_full:  # toutes les offres sans 5G
            if (C, O1, O2) not in f_data:
                f_data[C, O1, O2] = 0

model.f = pyo.Param(model.Cvec, model.O, model.O, initialize=f_data)  # taux de migration
            ...
            for o in model.O:
                ...
                for i_prev in model.I:
                    for o_prev in model.O:
                        M_val += pyo.value(model.f[C, o_prev, o]) * pyo.value(model.u_a[a])
                M[(a, C, o)] = M_val
    return M
...
def y_le_S(m, t, a, o, *C):
    if t == min(m.T): return pyo.Constraint.Skip
    S_expr = sum(model.f[C, o_prev, o] * m.u[t-1, a, i_prev, o_prev]
